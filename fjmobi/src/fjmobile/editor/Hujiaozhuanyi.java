...
public class Hujiaozhuanyi extends EditorPart {
	...
	public static final String ID = "fjmobile.editor.Hujiaozhuanyi"; //$NON-NLS-1$
	...
	public void createPartControl(Composite parent) {
		...
		button_3.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(final SelectionEvent e) {
				...
				InputDialog input = new InputDialog(e.display.getActiveShell(),
						"输入转移号码", "", null, new IInputValidator() {
							@Override
							public String isValid(String newText) {
								if (newText.length() < 8
										|| newText.length() > 12)
									return "输入正确的移动电话或者固定电话号码";
								return null;
							}
						});
				input.open();
				phone = input.getValue();
				e.display.syncExec(new Runnable() {
					public void run() {
						refresh("hujiaozhuanyi", "1");
					}
				});
				setTime(label_1);
			}
		});
		button_3.setText("设置转移");

		/*
		 * final Button button_4 = new Button(group, SWT.NONE);
		 * button_4.addSelectionListener(new SelectionAdapter() { public void
		 * widgetSelected(final SelectionEvent e) { e.display.syncExec(new
		 * Runnable() { public void run() { refresh("hujiaozhuanyi"); } });
		 * setTable(); } }); button_4.setText("取消转移");
		 */
		...
		button_4.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(final SelectionEvent e) {
				...
				e.display.syncExec(new Runnable() {
					public void run() {
						getMobiles();
						refresh("hujiaozhuanyi", "0");
					}
				});
				setTime(label_1);
			}
		});
		button_4.setText("取消转移");
		...
		button_2.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(final SelectionEvent e) {
				if (!DBFactory.clearAll("hujiaozhuanyi")) {
		...
		String time = DBFactory.getProp("hujiaozhuanyi_time");
		...
		actionList.add("hujiaozhuanyi_retain");
	...
	private void setTime(Label l) {
		...
		DBFactory.Sysprop("hujiaozhuanyi_time", time);
		l.setText(time);
	}
	...
	private void refresh(String type, String operate) {
		Map map = new HashMap();
		if (phone.length() == 11)
			map.put("ydphone", phone);
		else
			map.put("gdphone", phone);
		map.put("action", operate);
