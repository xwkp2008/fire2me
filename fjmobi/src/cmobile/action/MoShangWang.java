...
public class MoShangWang extends AbstractAction {
	...
	public String action(Map rs) throws HttpException, IOException {
		...
		String operateStr = "/service/operate/other/modifyService.do?tab=2&action=100203&status=1&number=o_service.o_mo_sjswkg&again=0";
		if (!open)
			operateStr = "/service/operate/other/modifyService.do?tab=2&action=100203&status=0&number=o_service.o_mo_sjswkg&again=0";
		String key = null;
	...
	public boolean judge(Map params) throws HttpException, IOException {
		...
		HttpCore
				...
				.doGet(
						Constant.URL
								+ "/service/operate/other/queryService.do?action=100203&number=o_service.o_mo_sjswkg&tab=2",
						new Callback() {
							@Override
