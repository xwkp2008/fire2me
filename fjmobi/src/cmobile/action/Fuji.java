...
public class Fuji extends AbstractAction {
	...
	public String action(Map rs) throws HttpException, IOException {
		...
		key = HttpCore
				...
				.doGet(
						Constant.URL
						// +
						// "/service/operate/other/stopPhoneCall.do?number=o_service.o_tjfj&status=fj&tab=2",
								+ "/service/operate/other/stopPhoneCall.do?number=o_service.o_fj&status=fj&tab=2",
						null);
				...
				new Callback() {
					...
					public void excute(HttpClient client, Object o) {
						try {
							...
							String rs = ParseFactory.parse("fuji", html);
							key.append(rs);
